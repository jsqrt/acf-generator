...
const changeDeepProperty = ({
		...
		switch (groupKeys.length) {
			case 1: {
				if (data[currentPageKey].fields[groupKeys[0]].fieldNames[fieldName]) {
					data = produce(data, draft => {
						draft[currentPageKey].fields[groupKeys[0]].fieldNames[fieldName] += 1;
					});
				} else {
					data = produce(data, draft => {
						draft[currentPageKey].fields[groupKeys[0]].fieldNames[fieldName] = 1;
			...
			case 2: {
				if (data[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].fieldNames[fieldName]) {
					data = produce(data, draft => {
						draft[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].fieldNames[fieldName] += 1;
					});
				} else {
					data = produce(data, draft => {
						draft[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].fieldNames[fieldName] = 1;
			...
			case 3: {
				if (data[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].sub_fields[groupKeys[2]].fieldNames[fieldName]) {
					data = produce(data, draft => {
						draft[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].sub_fields[groupKeys[2]].fieldNames[fieldName] += 1;
					});
				} else {
					data = produce(data, draft => {
						draft[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].sub_fields[groupKeys[2]].fieldNames[fieldName] = 1;
	...
	if (fieldConfig) {
		switch (groupKeys.length) {
			case 1: {
				data = produce(data, draft => {
					draft[currentPageKey].fields[groupKeys[0]].sub_fields[fieldKey] = fieldConfig;
				});
				break;
			}
			case 2: {
				data = produce(data, draft => {
					draft[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].sub_fields[fieldKey] = fieldConfig;
				});
				break;
			}
			case 3: {
				data = produce(data, draft => {
					draft[currentPageKey].fields[groupKeys[0]].sub_fields[groupKeys[1]].sub_fields[groupKeys[2]].sub_fields[fieldKey] = fieldConfig;
				});
				break;
			}
			default: break;
